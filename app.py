...
# ─── Подробные описания признаков (для подсказок и страницы "Все признаки") ───
...
if page == "Главная":
    ...
    **Год:** 2025–2026
    ...
    Модель обучена на более чем 11 000 примерах и показывает очень высокую точность.
    ...
    for i, feature in enumerate(base_features):
        with col1 if i % 2 == 0 else col2:
            val = st.selectbox(
                label=feature,
                options=["-1", "0", "1"],
                index=1,  # по умолчанию "1" (нормально)
...
# ─── Все признаки (подробно) ──────────────────────────────────────────────────
    ...
    for feature in base_features:
        with st.expander(f"**{feature}** — {feature_descriptions[feature]}"):
            ...
            elif feature == 'having_IP_Address':
                st.markdown("""
                - **-1** → URL содержит IP-адрес (например, http://192.168.1.1) — **очень подозрительно**  
                - **1** → Используется нормальное доменное имя
                """)
                
            elif feature == 'URL_Length':
                st.markdown("""
                - **-1** → Короткий или нормальный URL  
                - **1** → Очень длинный URL (более 75 символов) — часто используется в фишинге
                """)
                
            elif feature == 'Prefix_Suffix':
                st.markdown("""
                - **-1** → Нет дефиса в домене  
                - **1** → Есть дефис в домене (например, pay-pal-login.com) — часто подделка
                """)
                
            elif feature == 'Domain_registeration_length' or feature == 'age_of_domain':
                st.markdown("""
                - **-1** → Домен зарегистрирован недавно (менее года) — большой риск  
                - **1** → Домен старый и зарегистрирован давно — вызывает доверие
                """)
                
            elif feature in ['web_traffic', 'Page_Rank', 'Google_Index', 'Links_pointing_to_page']:
                st.markdown("""
                - **-1** → Низкий трафик / низкий ранг / не проиндексирован / мало внешних ссылок  
                - **0** → Средние показатели  
                - **1** → Высокий трафик / высокий ранг / проиндексирован / много ссылок
                """)
...
# ─── О модели ─────────────────────────────────────────────────────────────────
    ...
    **Топ-важные признаки (по убыванию):**
    ...
    лучше иногда ошибочно заподозрить легитимный сайт, чем пропустить настоящий фишинг.
    """)
